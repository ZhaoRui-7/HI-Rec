...
def extract_data(items):
    ...
    item_ids = []
    for item in items:
        embeddings.append(item['user_emb'])
        item_ids.append(item['user_id'])
...
def recursive_clustering(embeddings, item_ids, num_clusters, depth, result, prefix=''):
    ...
    if len(unique_embeddings) < len(embeddings):
        print("here")
    if len(unique_embeddings) < num_clusters:
        num_clusters = len(unique_embeddings)
...
def bottom_up_clustering(embeddings, item_ids, initial_num_clusters, final_num_clusters, depth, result):
    ...
    for i in range(final_num_clusters):
        cluster_indices = np.where(labels == i)[0]
        result[f'final_cluster_{i}'] = [current_item_ids[idx][0][2] for idx in cluster_indices]
