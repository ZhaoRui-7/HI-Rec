...
def movie_data_processing(path, title_length = 16):
    ...
    movies = pd.read_table(os.path.join(path, 'movie_map.dat'), sep='::', encoding='ISO-8859-1',
...
def rating_data_processing(path):
    ...
    ratings = pd.read_table(os.path.join(path, 'rating_map.dat'), sep='::',
        header=None, names=ratings_title, engine='python')
    # 保留每个UserID的前10条评分记录
    ratings = ratings.sort_values(by=['UserID', 'timestamps']).groupby('UserID').head(10)
...
def split_train_test(feature, targets, data, users, movies):
    ...
    # x_train, x_test, y_train, y_test = train_test_split(feature, targets, test_size=0.05, random_state=2022)#x是输入特征，y是label
    # x_train, x_val, y_train, y_val = train_test_split(x_train, y_train, test_size=0.05, random_state=2022)
    ...
    for userid in unique_userids:
        ...
        rating = 1 if user_data['ratings'].values[0]>3 else 0
        ...
        'Gender':user_data['Gender'].values[0], 'Age': user_data['Age'].values[0],
        'JobID': user_data['JobID'].values[0], 'Title': user_data['Title'].values[0],
    ...
    for itemid in unique_itemids:
        ...
        rating = 1 if item_data['ratings'].values[0] > 3 else 0
        ...
        'Gender':item_data['Gender'].values[0], 'Age': item_data['Age'].values[0],
        'JobID': item_data['JobID'].values[0], 'Title': item_data['Title'].values[0],
