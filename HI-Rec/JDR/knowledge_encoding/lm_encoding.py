...
def get_history_text(data_path):
    ...
    for piece in raw_data:
        ...
        pure_hist = prompt.split('The movies the user has previously watched are as follows:')[-1].strip()
...
def get_item_text(data_path):
    ...
    for piece in raw_data:
        idx, prompt, answer = piece
        pure_hist = prompt.split('The users like this movie are as follows:')[-1].strip()
        text_list.append(pure_hist+answer)
        idx_list.append(idx)
    return idx_list, text_list


def get_text_data_loader(data_path, batch_size):
    hist_idxes, history = get_history_text(os.path.join(data_path, 'path to user klg'))
    print('chatgpt.hist 1', history[1], 'hist len', len(history))
    item_idxes, items = get_item_text(os.path.join(data_path, 'path to item klg'))
...
def inference(model, tokenizer, dataloader, model_name, aggregate_type):
    ...
    with torch.no_grad():
        for x in tqdm(dataloader):
            ...
            pred = get_paragraph_representation(model_name, outputs, mask, aggregate_type)
...
def main(knowledge_path, data_path, model_name, batch_size, aggregate_type):
    ...
    if model_name == 'chatglm':
        checkpoint = 'path to chatglm-6b' if os.path.exists('path to chatglm-6b') else 'chatglm-6b'
    ...
    # save_json(item_vec_dict, os.path.join(data_path, 'sft_a_{}_{}_augment.item'.format(model_name, aggregate_type)))
