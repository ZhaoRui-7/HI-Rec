...
def get_paragraph_representation(model_name,outputs, mask, pooler='cls', dim=1):
    ...
    if pooler == 'cls':
        ...
        if model_name == 'bert' or model_name == 'longformer':
            return ((last_hidden * mask.unsqueeze(-1)).sum(dim) / mask.sum(dim).unsqueeze(-1)).cpu()
        else:
            last_hidden = torch.mean(last_hidden, dim=1).cpu()
            return last_hidden
        #return ((last_hidden * mask.unsqueeze(-1)).sum(dim) / mask.sum(dim).unsqueeze(-1)).cpu()
