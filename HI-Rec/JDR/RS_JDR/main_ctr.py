...
from models import DeepInterestNet, DCN, DeepFM, DIEN, xDeepFM, FiBiNet, FiGNN, AutoInt, GRU4Rec
...
def eval(model, test_loader):
    ...
    for i in range(len(preds)):
        if (preds[i][0]>=0.5 and labels[i] ==1) or (preds[i][0]<0.5 and labels[i] ==0):
            acc+=1
    # binary_preds = (preds >= 0.5).astype(int)
    print(acc,len(preds))
    accuracy = acc/len(preds)
    return auc, accuracy, ll, np.mean(losses), eval_time


def test(args):
    model = torch.load(args.reload_path)
    test_set = AmzDataset(args.data_dir, 'test', args.task, args.max_hist_len, args.augment, args.aug_prefix, args.dense_dim)
    test_loader = Data.DataLoader(dataset=test_set, batch_size=args.batch_size, shuffle=False)
    print('Test data size:', len(test_set))
    auc, acc, ll, loss, eval_time = eval(model, test_loader)
    print("test loss: %.5f, test time: %.5f, auc: %.5f,acc: %.5f, logloss: %.5f" % (loss, eval_time, auc,acc, ll))
...
def train(args):
    train_set = AmzDataset(args.data_dir, 'train', args.task, args.max_hist_len, args.augment, args.aug_prefix, args.dense_dim)
    test_set = AmzDataset(args.data_dir, 'test', args.task, args.max_hist_len, args.augment, args.aug_prefix, args.dense_dim)
    ...
    for epoch in range(args.epoch_num):
        ...
        for _, data in enumerate(train_loader):
            ...
            #loss = outputs['loss']+outputs['reconstruction_loss']
            #else:
            loss = outputs['loss']
            optimizer.zero_grad()
            loss.backward()
        ...
        eval_auc, eval_acc, eval_ll, eval_loss, eval_time = eval(model, test_loader)
        print("EPOCH %d  STEP %d train loss: %.5f, train time: %.5f, test loss: %.5f, test time: %.5f, auc: %.5f, acc: %.5f, "
              "logloss: %.5f" % (epoch, global_step, np.mean(train_loss), train_time, eval_loss,
                                 eval_time, eval_auc, eval_acc, eval_ll))
...
def parse_args():
    ...
    parser.add_argument('--dense_dim', default=768, type=int, help='augment_vector_dim')

    parser.add_argument('--use_autoencoder', default='true', type=str, help='whether use autoencoder')
    parser.add_argument('--share_autoencoder_arch', default='360,200,128', type=str2list, help='autoencoder_arch')
    parser.add_argument('--dim_reduction', default=64, type=int, help='output dimension of autoencoder')

    args, _ = parser.parse_known_args()
    args.augment = True if args.augment.lower() == 'true' else False
    args.use_autoencoder = True if args.use_autoencoder.lower() == 'true' else False
