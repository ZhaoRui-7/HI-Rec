...
class Autoencoder(nn.Module):
    def __init__(self, shared_autoencoder_arch, input_dim1, input_dim2, output_dim1, output_dim2, dropout):
        super(Autoencoder, self).__init__()
        self.inp1 = nn.Linear(input_dim1,shared_autoencoder_arch[0])
        self.inp2 = nn.Linear(input_dim2,shared_autoencoder_arch[0])
        self.encoder = MLP(shared_autoencoder_arch[1:], shared_autoencoder_arch[0], dropout)
        self.out1 = nn.Linear(shared_autoencoder_arch[-1],output_dim1)
        self.out2 = nn.Linear(shared_autoencoder_arch[-1],output_dim2)
        self.decoder1 = MLP(shared_autoencoder_arch[::-1]+[input_dim1], output_dim1, dropout)
        self.decoder2 = MLP(shared_autoencoder_arch[::-1]+[input_dim2], output_dim2, dropout)
    
    def forward(self, x, type):
        if  type == 'only_encoder':
            x = self.inp1(x)
            encoded = self.encoder(x)
            encoded = self.out1(encoded)
            decoded = self.decoder1(encoded)
        elif type == 'co_encoder':
            x = self.inp2(x)
            encoded = self.encoder(x)
            encoded = self.out2(encoded)
...
class BaseModel(nn.Module):
    def __init__(self, args, dataset):
        ...
        if self.augment_num:
            if args.use_autoencoder :
                self.share_ae_arch = args.share_autoencoder_arch
                self.dim_reduction = args.dim_reduction
                if self.algo == 'DIN' or self.algo == 'DIEN':
                    self.autoencoder = Autoencoder(self.share_ae_arch, self.dense_dim, self.dense_dim+self.itm_emb_dim,
                                               self.dim_reduction, self.dim_reduction+self.itm_emb_dim, self.convert_dropout)
                else:
                    self.autoencoder = Autoencoder(self.share_ae_arch, self.dense_dim, self.dense_dim+self.hist_emb_dim,
                                               self.dim_reduction, self.dim_reduction+self.hist_emb_dim, self.convert_dropout)
                self.convert_module = ConvertNet(args, self.dim_reduction, self.convert_dropout, self.convert_type)
                self.dens_vec_num = args.convert_arch[-1] * self.augment_num
            else:
                self.share_ae_arch = None
    ...
    def process_input(self, inp):
        ...
        if self.task == 'ctr':
            ...
            if self.augment_num:
                ...
                if self.share_ae_arch is not None :
                    autoencoder_vec = [self.autoencoder(x, 'only_encoder') for x in orig_dens_vec]
                    dens_vec = [autoencoder_vec[0][0]]
                    dens_vec = self.convert_module(dens_vec)
                else:
                    autoencoder_vec = None
                    dens_vec = self.convert_module(orig_dens_vec)
            else:
                dens_vec, orig_dens_vec, autoencoder_vec = None, None, None
            return item_emb, hist_emb, hist_len, dens_vec, orig_dens_vec, autoencoder_vec, labels
    ...
    def get_filed_input(self, inp):
        item_embedding, user_behavior, hist_len, dens_vec, orig_dens_vec, autoencoder_vec, labels = self.process_input(inp)
        user_behavior = torch.mean(user_behavior, dim=1).view(-1, self.hist_emb_dim)
        if self.share_ae_arch is not None :
            co_orig_dens_vec = [torch.cat([user_behavior,x], dim=-1) for x in orig_dens_vec]
            co_autoencoder_vec = [self.autoencoder(x, 'co_encoder') for x in co_orig_dens_vec]
        ...
        if self.share_ae_arch is not None :
            reconstruction_loss1 = nn.MSELoss()(autoencoder_vec[0][1], orig_dens_vec[0]) #+nn.MSELoss()(autoencoder_vec[1][1], orig_dens_vec[1])
            reconstruction_loss2 = nn.MSELoss()(co_autoencoder_vec[0][1], co_orig_dens_vec[0]) #+nn.MSELoss()(co_autoencoder_vec[1][1], co_orig_dens_vec[1])
            #print('reconstruction_loss:', reconstruction_loss1, reconstruction_loss2)
            # out['reconstruction_loss'] = reconstruction_loss1 + reconstruction_loss2
            return out, labels, reconstruction_loss1 + reconstruction_loss2
        else:
            return out, labels, 0
...
class GRU4Rec(BaseModel):
    ...
    def get_input_dim(self):
        return self.itm_emb_dim*2 + self.hist_emb_dim + self.hidden_size + self.dens_vec_num
...
class DeepInterestNet(BaseModel):
    ...
    def forward(self, inp):
        """
            ...
            :return score (bs)
        """
        query, user_behavior, hist_len, dens_vec, orig_dens_vec, autoencoder_vec, labels = self.process_input(inp)
        ...
        if self.share_ae_arch is not None :
            co_orig_dens_vec = [torch.cat([user_interest,x], dim=-1) for x in orig_dens_vec]
            co_autoencoder_vec = [self.autoencoder(x, 'co_encoder') for x in co_orig_dens_vec]
        ...
        if self.share_ae_arch is not None :
            ...
            reconstruction_loss2 = nn.MSELoss()(co_autoencoder_vec[0][1], co_orig_dens_vec[0]) #+nn.MSELoss()(co_autoencoder_vec[1][1], co_orig_dens_vec[1])
            #print('reconstruction_loss:', reconstruction_loss1, reconstruction_loss2)
            #distance = torch.norm(autoencoder_vec[0][1]-orig_dens_vec[0],p=2)
            #logloss = -torch.log(distance + 1e-8)
            out['reconstruction_loss'] = reconstruction_loss1 + reconstruction_loss2 #+ logloss
        return out
...
class DIEN(BaseModel):
    ...
    def forward(self, inp):
        """
            ...
            :return score (bs)
        """
        query, user_behavior, length, dens_vec, orig_dens_vec, autoencoder_vec, labels = self.process_input(inp)
...
class DCN(BaseModel):
    ...
    def forward(self, inp):
        ...
        if self.share_ae_arch is not None :
            ...
            reconstruction_loss2 = nn.MSELoss()(co_autoencoder_vec[0][1], co_orig_dens_vec[0]) #+nn.MSELoss()(co_autoencoder_vec[1][1], co_orig_dens_vec[1])
            #print('reconstruction_loss:', reconstruction_loss1, reconstruction_loss2)
            outputs['reconstruction_loss'] = reconstruction_loss1 + reconstruction_loss2
        return outputs
...
class xDeepFM(BaseModel):
    ...
    def forward(self, inp):
        inp, labels, rloss = self.get_filed_input(inp)
        ...
        outputs = self.get_ctr_output(logits, labels)
        outputs['reconstruction_loss'] = rloss
        return outputs
...
class FiBiNet(BaseModel):
    ...
    def forward(self, inp):
        feat_embed, labels, rloss = self.get_filed_input(inp)
